...
class _HomePageState extends State<HomePage> {
  ...
  /// Lưu progress thật theo courseId
  final Map<String, int> _courseProgressMap = {};
  ...
  Future<void> _loadData() async {
    try {
      ...
      final loadedCourses = results[1] as List<Course>;
      final loadedTopics = results[2] as List<TopicModel>;
      final loadedOnboarding = results[3] as Map<String, dynamic>?;

      final Map<String, int> computedProgress = {};

      for (final course in loadedCourses) {
        try {
          final List<Lesson> lessons =
          await LessonService.getLessonsByCourse(course.id);

          final int totalLessons = lessons.length;

          if (totalLessons == 0) {
            computedProgress[course.id] = 0;
            continue;
          }
          ...
          computedProgress[course.id] = percent.clamp(0, 100);
        } catch (e) {
          debugPrint(
            "Error computing progress for course ${course.id}: $e",
          );
          computedProgress[course.id] =
              course.progressPercent.clamp(0, 100);
        }
      }
      ...
      setState(() {
        ...
        topics = loadedTopics;
        myOnboarding = loadedOnboarding;
        _courseProgressMap
          ..clear()
          ..addAll(computedProgress);
        isLoading = false;
      });
    } catch (e) {
      debugPrint("Error loading data: $e");
  ...
  int _getCourseProgress(Course course) {
    return _courseProgressMap[course.id] ??
        course.progressPercent.clamp(0, 100);
  }
  ...
  Widget _buildHomeContent() {
    ...
    List<Course> inProgress = courses.where((c) {
      final progress = _getCourseProgress(c);
      return progress > 0 && progress < 100;
    }).toList();

    inProgress.sort((a, b) {
      final aProgress = _getCourseProgress(a);
      final bProgress = _getCourseProgress(b);
    ...
    if (inProgress.length >= 2) {
      ...
      List<Course> notStarted = courses.where((c) {
        final progress = _getCourseProgress(c);
        return progress == 0 && !displayContinuing.contains(c);
      }).toList();
                    ...
                    Row(
                      children: [
                        Expanded(
                          child: continueCard(
                            displayContinuing[0].title,
                            "${_getCourseProgress(displayContinuing[0])}%",
                            _getCourseProgress(displayContinuing[0]) / 100,
                            const Color(0xFF5F2EFF),
                            theme,
                                () => _openCourse(displayContinuing[0]),
                          ),
                        ...
                        Expanded(
                          child: displayContinuing.length > 1
                              ? continueCard(
                            displayContinuing[1].title,
                            "${_getCourseProgress(displayContinuing[1])}%",
                            _getCourseProgress(displayContinuing[1]) / 100,
                            Colors.orange,
                            theme,
                                () => _openCourse(displayContinuing[1]),
                          )
