...
class _ProgressPageState extends State<ProgressPage> {
  ...
  Future<void> loadAllData() async {
    ...
    try {
      ...
      final loadedCourses = results[0] as List<Course>;
      final loadedStreak = results[1] as Map<String, dynamic>?;
      final loadedStats = results[2] as StatsResponse?;

      /// Tính progress thật từ lesson
      ...
      for (final course in loadedCourses) {
        try {
          ...
          debugPrint("Error computing progress for course ${course.id}: $e");

          /// Nếu lỗi API lesson của course nào đó thì fallback về progress cũ
          computedProgress[course.id] = course.progressPercent.clamp(0, 100);
        }
      }
  ...
  int _getCourseProgress(Course course) {
    return _courseProgressMap[course.id] ?? course.progressPercent.clamp(0, 100);
  }
  ...
  List<Course> _getLearningCourses() {
    ...
    filtered.sort((a, b) {
      ...
      final aCompleted = aProgress >= 100;
      final bCompleted = bProgress >= 100;
              ...
              child: Column(
                ...
                children: [
                  ...
                  if (learningCourses.isEmpty)
                    ...
                    ...learningCourses
                        .map((course) => _buildCourseItem(course, theme)),
                ],
              ),
            ),
          ),
  ...
  Widget _buildCourseItem(Course course, ThemeData theme) {
    final int progressPercent = _getCourseProgress(course);
