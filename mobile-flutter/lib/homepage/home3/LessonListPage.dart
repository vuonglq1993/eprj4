...
// // import 'quiz_page.dart'; data fake muốn tạo riêng cho progress trang khác để tránh lỗi đến các trang khác
// // import 'lesson_detail_page.dart'; // Import trang chi tiết nếu bạn đã có
// import '../quiz/quiz_progress.dart';
//
...
//       context,
//       MaterialPageRoute(
//         builder: (context) => QuizProgressPage( // Dùng trang thật ở trên
//           courseId: widget.courseId,
//           lessonId: lesson.id,
//         ),
//       ),
//     ).then((_) {
//       // Khi từ Quiz quay lại, load lại danh sách để hiện tích xanh (COMPLETED)
//       setState(() {
...
import 'package:flutter/material.dart';
import '../../models/lesson_model.dart';
import '../../services/lesson_service.dart'; // Đảm bảo dùng LessonService
...
class _LessonListPageState extends State<LessonListPage> {
  // Sửa thành List<Lesson> rõ ràng để Flutter không bị nhầm lẫn
                ...
                itemBuilder: (context, index) {
                  ...
                  return GestureDetector(
                    ...
                    child: Container(
                      ...
                      child: Row(
                        children: [
                          _buildStatusIcon(index + 1, isCompleted),
                          const SizedBox(width: 15),
                          Expanded(
                            child: Column(
                              crossAxisAlignment: CrossAxisAlignment.start,
  ...
  Widget _buildStatusIcon(int index, bool isCompleted) {
    return Container(
      width: 40,
      height: 40,
      decoration: BoxDecoration(
      ...
      child: Center(
        child: isCompleted
            ...
            : Text(
          "$index",
          style: const TextStyle(color: Color(0xFF5F2EFF), fontWeight: FontWeight.bold),
        ),
      ),
    );
  }
  ...
  Widget _buildLessonInfo(Lesson lesson) {
    return Row(
      children: [
        const Icon(Icons.timer_outlined, size: 14, color: Colors.grey),
        const SizedBox(width: 4),
        Text("${lesson.durationMinutes} phút", style: const TextStyle(color: Colors.grey, fontSize: 13)),
        const SizedBox(width: 12),
        const Icon(Icons.assignment_outlined, size: 14, color: Colors.grey),
        const SizedBox(width: 4),
        Text(lesson.type, style: const TextStyle(color: Colors.grey, fontSize: 13)),
      ],
    );
  }
