...
class _ProfilePageState extends State<ProfilePage> {
  ...
  Future<void> _fetchCompletedCourses() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        completedCourses =
            allCourses.where((c) => (c.progressPercent) >= 100).toList();
      }
    } catch (e) {
  ...
  List<Course> _getPreviewAchievements() {
    if (completedCourses.isEmpty) return [];
    return completedCourses.take(2).toList();
  }

  void _openAllAchievements() {
    if (completedCourses.isEmpty) return;
  ...
  Widget build(BuildContext context) {
    return ValueListenableBuilder<ThemeMode>(
      ...
      builder: (context, mode, child) {
        final theme = Theme.of(context);
        final isDark = mode == ThemeMode.dark;
        final previewAchievements = _getPreviewAchievements();
                      ...
                      Padding(
                        ...
                        child: Column(
                          children: [
                            ...
                            _buildSectionHeader(
                              "Achievement",
                              _openAllAchievements,
                              theme,
                              canOpen: completedCourses.isNotEmpty,
                            ),
                            const SizedBox(height: 15),
                            ...
                            completedCourses.isEmpty
                              ...
                              itemBuilder: (context, index) {
                                final course = previewAchievements[index];
                                return _buildAchievementCard(
                                  course.title,
                                  "Level ${course.level}",
                                  _getFlagEmoji(course.title),
      ...
      children: [
        ...
        GestureDetector(
          ...
          child: Text(
            "View All",
            style: TextStyle(
              color: canOpen ? const Color(0xFF5F2EFF) : Colors.grey,
              fontWeight: FontWeight.w600,
              fontSize: 13,
            ),
          ),
