...
public class NotificationController {
    ...
    @Operation(summary = "Gửi test notification đến chính user hiện tại")
    @PostMapping("/test")
    public ResponseEntity<Void> sendTest(@AuthenticationPrincipal UserPrincipal principal) {
        pushService.sendToUser(
                principal.getUserId().toString(),
                "📚 Đến giờ học rồi!",
                "Hôm nay bạn đã học chưa? Mở app và chinh phục bài học mới nào!",
                java.util.Map.of("type", "STUDY_REMINDER")
        );
        return ResponseEntity.ok().build();
    }

    @Operation(summary = "Lấy cài đặt reminder")
    ...
    @Operation(summary = "Lưu cài đặt reminder")
    @PutMapping("/reminder-settings")
    public ResponseEntity<ReminderSettingsResponse> saveReminderSettings(
            @Valid @RequestBody ReminderSettingsRequest req,
            @AuthenticationPrincipal UserPrincipal principal) {
        try {
            String userId = principal.getUserId().toString();
